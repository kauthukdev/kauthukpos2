...
class ProductsImport implements ToModel, WithHeadingRow, WithChunkReading
{
    ...
    public function model(array $row)
    {
        ...
        // Check for duplicate: same title + same category
        if ($categoryId && !empty(trim($row['title'] ?? ''))) {
            $existing = Product::where('title', trim($row['title']))
                ->where('category', $categoryId)
                ->first();
            if ($existing) {
                // Build the new data to compare
                $newData = [
                    ...
                    'stock_status'  => isset($row['stock_status']) ? (int) $row['stock_status'] : 1,
                ];

                // Check if any detail has changed
                $hasChanges = false;
                $changedFields = [];
                foreach ($newData as $field => $newValue) {
                    $oldValue = $existing->{$field};
                    if ($field === 'hsncode') {
                        // String comparison for hsncode
                        if ((string) ($oldValue ?? '') !== (string) ($newValue ?? '')) {
                            $hasChanges = true;
                            $changedFields[] = $field;
                        }
                    } else {
                        // Numeric comparison for prices, gst, stock
                ...
                if ($hasChanges) {
                    ...
                    try {
                        ...
                        $rowData['errors'] = '';
                        $rowData['status'] = 'Updated (' . implode(', ', $changedFields) . ')';
                        $this->results[] = $rowData;
                        $this->updatedCount++;
                        Log::info("Updated existing product: {$row['title']} (ID: {$existing->id}), changed: " . implode(', ', $changedFields));
                    } catch (\Exception $e) {
                        $rowData['errors'] = 'Failed to update: ' . $e->getMessage();
                    ...
                    // All details are the same — skip with error
                    $errors[] = "Product '{$row['title']}' already exists in category '{$categoryName}' with identical details (Code: {$existing->product_code}). No changes detected.";
                }
            }
        }
    ...
    private function generateProductCode($category)
    {
        try {
            ...
            return $abbreviation . '-' . time() . rand(10, 99);
