...
{ 
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        try {
            ...
            \Log::info("Bulk deleted {$count} products by user: " . auth()->id());
            ...
            return redirect()->back()->withErrors(['error' => 'Failed to delete products: ' . $e->getMessage()]);
        }
    }
    ...
    public function processBulkUpload(Request $request)
    {
        ...
        try {
            ...
            if ($importedCount > 0 || $updatedCount > 0) {
                $parts = [];
                if ($importedCount > 0) {
                    $parts[] = "{$importedCount} product(s) imported";
                }
                if ($updatedCount > 0) {
                    $parts[] = "{$updatedCount} product(s) updated";
                }
