...
export default function Create({ auth, products }) {
    ...
    const isInterstate = data.is_interstate;
    ...
    // Calculate totals whenever items change or general_discount changes
    useEffect(() => {
        let subtotal = 0;
        let totalTax = 0;
        let cgst = 0;
        ...
        const roundedDiscount = Math.round(data.general_discount || 0);
        const finalGrandTotal = Math.max(0, grandTotal - roundedDiscount);
    ...
    }, [data.items, data.general_discount, isInterstate]);
    ...
    // Check invoice number existence with debounce
    useEffect(() => {
        const checkInvoiceNumber = async () => {
            ...
            try {
                const response = await axios.get(route('sales.check-invoice'), {
                    params: { invoice_no: invoiceNo }
                });
                
                if (response.data.exists) {
                    setInvoiceNoError('This invoice number already exists. Please use a different number.');
                } else {
                    setInvoiceNoError('');
                }
            } catch (error) {
    ...
    const handleConfigSelect = (key, value) => {
        ...
        if (newConfig.type && (key === 'region' || newConfig.region)) {
            const selectedRegion = key === 'region' ? value : newConfig.region;
            setShowConfigModal(false);
            setData('is_interstate', selectedRegion === 'Interstate');
            generateInvoiceDetails(newConfig.type, selectedRegion);
        }
    };
                        ...
                        <div className="p-6 text-gray-900">
                            <form onSubmit={handleSubmit}>
                                    ...
                                    <div className="border p-4 rounded-lg">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                {invoiceNoError && (
                                                    <p className="mt-2 text-sm text-red-600">{invoiceNoError}</p>
                                                )}
                                                {isCheckingInvoice && (
                                        ...
                                        <table className="min-w-full bg-white border table-fixed">
                                            <thead>
                                                <tr className="bg-gray-100 text-gray-700 uppercase text-sm leading-normal">
                                                    ...
                                                    {isInterstate ? (
                                                        <th className="py-2 px-2 text-center w-24">IGST</th>
                                                    ) : (
                                                        <>
                                            ...
                                            <tbody className="text-gray-600 text-sm">
                                                {data.items.map((item, index) => {
                                                    ...
                                                    const afterDiscount = itemAmount - discountAmount;
                                                    const gstAmount = (afterDiscount * item.gst_percentage) / 100;
                                                    const itemIgst = isInterstate ? gstAmount : 0;
                                                    const itemCgst = isInterstate ? 0 : gstAmount / 2;
                                                    const itemSgst = isInterstate ? 0 : gstAmount / 2;
                                            ...
                                            <div className="border p-4 rounded-lg">
                                                ...
                                                <div className="flex justify-between py-2 border-b">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <input
                                                            type="text"
                                                            className="w-24 text-right border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-1 px-2"
                                                            value={data.general_discount || 0}
                                                            onChange={(e) => {
                                                                const value = e.target.value.replace(/[^0-9]/g, '');
                                                                const numValue = parseFloat(value) || 0;
                                                                const roundedValue = Math.round(numValue);
                                                                setData('general_discount', roundedValue);
                                                            }}
                                                            onKeyDown={(e) => {
                                ...
                                <div className="flex items-center justify-end mt-8 no-print">
                                    ...
                                    <button
                                        type="submit"
                                        className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                                        disabled={processing || invoiceNoError !== '' || isCheckingInvoice}
                                    >
